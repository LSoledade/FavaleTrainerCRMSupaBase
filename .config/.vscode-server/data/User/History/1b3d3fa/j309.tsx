...
export function RecurringAppointmentDialog({ isOpen, onClose }: RecurringAppointmentDialogProps) {
  ...
  // Handle day selection - using useCallback to prevent unnecessary re-renders
  ...
  // Handle time slot selection - using useCallback
  ...
  // Handle professor selection - using useCallback
  ...
  // Show loading state
  const isLoading = isLoadingLeads || isLoadingServices || isLoadingProfessors;

  // Show error state if any query failed
  const hasError = leadsError || servicesError || professorsError;
